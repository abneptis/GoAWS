...
func init(){
  ...
  Modules["sqs"].Calls["create"] = func(args []string)(err os.Error){
        if len(args) != 1 { return os.NewError("Usage: create QUEUE")}
  ...
  Modules["sqs"].Calls["list"] = func(args []string)(err os.Error){
        if len(args) != 0 { return os.NewError("Usage: list")}
  ...
  Modules["sqs"].Calls["drop"] = func(args []string)(err os.Error){
        if len(args) != 1 { return os.NewError("Usage: drop queue")}
        Q, err := s.CreateQueue(signer, args[0],  flag_default_timeout)
        if err == nil {
  ...
  Modules["sqs"].Calls["push"] = func(args []string)(err os.Error){
        if len(args) != 1 { return os.NewError("Usage: push queuename") }
        Q, err := s.CreateQueue(signer, args[0],  flag_default_timeout)
        if err == nil {
          var n int
  ...
  Modules["sqs"].Calls["rm"] = func(args []string)(err os.Error){
        if len(args) != 2 { return os.NewError("Usage: rm queuename receipthandle") }
        Q, err := s.CreateQueue(signer, args[0],  flag_default_timeout)
        if err == nil {
  ...
  Modules["sqs"].Calls["peek"] = func(args []string)(err os.Error){
        if len(args) != 1 { return os.NewError("Usage: peek queuename") }
        Q, err := s.CreateQueue(signer, args[0],  flag_default_timeout)
        var body []byte
        var id string
        if err == nil {
